...
The interface speaks. Every label, message, and empty state is a sentence in a conversation with
the user. Good UX writing is invisible — it removes friction without drawing attention to itself.
Pairs with the anti-AI-copy tells in [`TASTE.md`](./TASTE.md).

> Voice is constant (who we are). Tone adapts to context (how we say it in this moment). An error
> and a success both share the same voice, but their tone differs.
...
**Rule:** the higher the user's stress, the plainer the language. Never use humor in errors,
...
Buttons are verbs, not nouns. Label the *outcome*, not the mechanism: "Create account" beats
"Submit".
...
**Never:** expose stack traces, error codes alone, or "Oops!" with no path forward. An error
...
Distinguish **first-use** empty (guide them) from **cleared** empty (reassure them). The shadcn
`Empty` component is the home for these.
...
| **Tooltips** | Add info, never repeat the label | (on an info icon) "Visible only to your team" |
...
- **Sentence case** for everything — buttons, labels, headings, menus. ("Create new project", not
  "Create New Project")
- **No terminal punctuation** on labels, buttons, or single-sentence tooltips. Use periods in
  multi-sentence body and error messages.
...
- **Avoid** ALL CAPS for emphasis (reads as shouting, hurts screen readers), exclamation marks
  (max one per screen), and "please" in routine actions (it's filler).
...
- **No directional-only or sense-only instructions** ("click the button on the right", "the green
  one") — pair with a label. Ties to WCAG 1.3.3 (`A11Y.md`) and the no-color-only rule.
